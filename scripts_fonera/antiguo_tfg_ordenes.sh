...
echo "Transmisión de los .pcap en esta fonera a babosa cada $timer_tcpdump_interval segundos" >> /etc/archivos/log_errores
...
registrar() {
  # Primero miramos si está montado el SSHFS
  # if [ "$(df | grep 'jmmontes@babosa.it.uc3m.es:/srv/Capturas')" = "" -a "$(df | grep '/mnt/babosa')" = "" ]; then
    # if $sshfs_montado; then
    #   echo "SSHFS debería estar montado. Puede haber un problema en la red. Se procederá a reiniciar la fonera" && reboot
    # fi
  #   mkdir -p /mnt/babosa
  #   echo "SSHFS no montado. Se procederá a montar SSHFS para virtualizar instancia del servidor"    
  #   sshfs -ossh_command="ssh -i /root/.ssh/id_rsa",reconnect jmmontes@babosa.it.uc3m.es:/home/jmmontes/Escritorio/Servidor/Capturas/$MY_MAC/ /mnt/babosa && sshfs_montado=true || echo "$(date): error al montar sshfs" >> /etc/archivos/log_errores 
  # fi
  ...
  echo "Carga media del procesador: $(uptime | awk -F " " ' {print $9}')" >> /etc/archivos/$MY_MAC || (echo "$(date): error al determinar la carga media del procesador" >> /etc/archivos/log_errores && sleep 10 && reboot)
  echo "Directorio en babosa: $(df | grep -e @babosa)" >> /etc/archivos/$MY_MAC || (echo "$(date): error al determinar la ruta de montaje en babosa" >> /etc/archivos/log_errores && sleep 10 && reboot)
  echo "Directorio de babosa montado en: $(df | grep -e /mnt/babosa | awk -F " " ' {print $5}')" >> /etc/archivos/$MY_MAC || (echo "$(date): error al determinar la ruta de montaje en fonera" >> /etc/archivos/log_errores && sleep 10 && reboot)
  echo "*************************" >> /etc/archivos/$MY_MAC
  # scp -i /root/.ssh/id_rsa /etc/archivos/$MY_MAC jmmontes@babosa.it.uc3m.es:/home/jmmontes/Escritorio/Servidor/Registros || echo "$(date): error al copiar el archivo de registro al servidor" >> /etc/archivos/log_errores
  scp -i /root/.ssh/id_rsa /etc/archivos/$MY_MAC -o ConnectTimeout=10 jmmontes@babosa.it.uc3m.es:/srv/Registros || (echo "$(date): error al copiar el archivo de registro al servidor" >> /etc/archivos/log_errores && sleep 10 && reboot)
  check_tmpused
...
calc_thr_avg() {
  ...
  if [ "$(echo $thr '>' $thr_max | bc -l)" -eq 1 ]; then
    ...
    thr_max=$(echo "$thr" | bc)
    echo "$(date): el nuevo canal de máx tasa es $canal_maximo" >> /mnt/babosa/tiemposrastreo
  fi

  let timediff=$timer_rcv1-$timer_rcv
  echo "$canal_actual,$(date +%Y-%m-%d-%H-%M:%S), $thr kbps, $timediff seg en canal, teorico $time_average_interval, maximo de ronda ($thr_max, $canal_maximo)" >> /mnt/babosa/thr.csv
...
cambia_canal() {
  ...
  if [ $canal_actual -eq 13 ]; then
    ...
    do
      ...
      echo "El tiempo calculado para el canal $j es de $thrj segundos" #>> /mnt/babosa/tiemposrastreo
    ...
    echo "$(date): array de tiempos de capt es $TMRARRAY" >> /mnt/babosa/tiemposrastreo
    ...
    thr_max=$(echo 0 | bc)
    let nuevo_canal=1
    echo "$(date): sumatorio de tiempos de rotación es $sumtant, volviendo a canal 1" >> /mnt/babosa/tiemposrastreo
  else
  ...
  echo "$(date): siguiente canal $nuevo_canal durante $switch_freq_interval segundos" >> /mnt/babosa/tiemposrastreo
...
#
mandar_capturas() {
    echo "Enviando .pcap's a babosa"
    current_dir=$pwd
    cd /tmp/
    files_to_copy=$(ls -tr *.pcap | head -$( echo "$(ls *.pcap | wc -w)" | bc))
    echo $files_to_copy > files_to_copy
    timer_inicio="$(date +%s)"
    echo "$(date): comienzo de envio de $files_to_copy" >> /mnt/babosa/tiemposrastreo
    scp /tmp/$files_to_copy /mnt/babosa/ && rm /tmp/$files_to_copy && timer_fin="$(date +%s)" && (tcpdump -v -i mon0 -s 100 -w /tmp/captura-%Y-%m-%d-%H-%M-%S.pcap -G $captura_frequency -W 1 &) && let timer_dif=$timer_fin-$timer_inicio && echo "$(date): fin de envío, duracion $timer_dif, volviendo a capturar" >> /mnt/babosa/tiemposrastreo 
    cd $current_dir
...
#    
reinicia() {
  echo "Reiniciando scripts de monitorización"
  echo "Reiniciando scripts de monitorización" > rebootok
  cd /etc/init.d
  ./tfg_inicio start
}
...
#   ssh -i /root/.ssh/id_rsa jmmontes@babosa.it.uc3m.es "cd /home/jmmontes/Escritorio/Servidor/Capturas/$MY_MAC; sh compress.sh $capture_name" || echo "$(date): error al mandar orden de compresion de $capture_name al servidor" >> /etc/archivos/log_errores
#   cd /tmp
...
echo "Montando punto /mnt/babosa con SSHFS"
mkdir -p /mnt/babosa
# sshfs jmmontes@babosa.it.uc3m.es:/home/jmmontes/Escritorio/Servidor/Capturas/$MY_MAC/ /mnt/babosa -o ssh_command="ssh -i /root/.ssh/id_rsa",reconnect || (echo "$(date): error al montar sshfs. Reiniciando..." >> /etc/archivos/log_errores && sleep 400 && reboot)
sshfs jmmontes@babosa.it.uc3m.es:/srv/Capturas/$MY_MAC/ /mnt/babosa -o ssh_command="ssh -i /root/.ssh/id_rsa",reconnect || (echo "$(date): error al montar sshfs. Reiniciando..." >> /etc/archivos/log_errores && sleep 400 && reboot)
registrar
config_wireless
config_tcpdump
timer=0
bytes_rcv=$(ifconfig mon0 | grep "RX bytes" | awk -F ":" ' { print $2 }' | awk '{ print $1 }')
timer_rcv="$(date +%s)"
scp /etc/archivos/log_errores /mnt/babosa/ && echo "Transmisión del log de errores correcta" || echo "$(date): error en la copia del log de errores al servidor" >> /etc/archivos/log_errores
